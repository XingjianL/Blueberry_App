...
def run_ml_tflite(image, path):
    ...
    print(output_tf)
    output = np.squeeze(np.reshape(np.array(output_tf.getFloatArray()),output_shape))
    print(output.shape)
    print(np.max(output))
    ...
    if len(indices) > 0:
        for i in indices.flatten():
            ...
            print(x,y,x+w,y+h, img.shape, img.flags['WRITEABLE'])
    ...
    return img, len(radii), avg_area#, areas